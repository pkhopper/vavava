...
class HttpDownloadClipHandler(urllib2.BaseHandler):
    ...
    def __init__(self, fp, data_range=None, file_mutex=None,
        ...
        self.__checker = _re_compile('(\d+)[^\d](\d+)')
    ...
    def __assert_range(self, str):
        a, b = self.__checker.search(str).groups()
        a, b = int(a), int(b)
        assert a == self.data_range[0] and b == self.data_range[1]
    ...
    def __handle_all(self, req, resp):
        ...
        assert self.stop_ev.is_set() or self.offset == size
    ...
    def __handle_clip(self, req, resp):
        ...
        while not self.stop_ev.is_set():
            ...
            if self.callback and not self.fp.closed:
                self.callback(self.offset, data_len)
            self.offset += data_len
        assert self.stop_ev.is_set() or self.offset == self.end_at + 1
...
class HttpFetcher(HttpUtil):
    ...
    def fetch(self, url, fp, data_range=None, file_mutex=None,
              bs=HttpDownloadClipHandler.BUFFER_SIZE, callback=None, timeout=None):
        self.__fetching = True
        self.handler = HttpDownloadClipHandler(fp, data_range=data_range, file_mutex=file_mutex,
...
#
#         for clip in clips:
#             thread = MiniAxel.DownloadThread(url=url, fp=fp, data_range=clip, mutex=mutex,
...
#
#         def __init__(self, url, fp, data_range=None, mutex=None,
...
#             self.http_fetcher.setToStop()
#
#         def run(self):
#             while not self.isSetStop():
#                 try:
#                     self.http_fetcher.fetch(self.url, fp=self.fp, data_range=self.data_range,
